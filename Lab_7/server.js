...
app.get('/team_stats', function(req, res){
  var games = 'select * from football_games;';//write SQL statement to get all football games
  var wins = 'select visitor_score, home_score from football_games where visitor_score < home_score;';//SQL query for num of wins (home_score>visitor_score)
  var losses = 'select visitor_score, home_score from football_games where visitor_score > home_score;';//SQL query for num of losses (home_score < visitor_score)
    db.task('get-everything', task => {
      return task.batch([
        task.any(games),
        task.any(wins),
...
To exhausted, too many things due by Sunday, this is how far I got before giving up on part B:
...
app.get('/player_info/player', function(req, res) {
	...
	var player = "select name, year, major, passing_yards, rushing_yards from football_players where id = '"+player_id+"';"; // Retrieve the specific football player's informatioin from the football_players table
	var total_games = 'select count(*) from football_games where (select id from football_players where id = '+player_id+');';// Retrieve the total number of football games the player has played
	db.task('get-everything', task => {
        return task.batch([
            task.any(player_id),
            task.any(player),
