...
*/
	function loadStatsPage(){
		...
		{//Outer for loop iterates over each row
			console.log(table.rows[row_counter].cells);
			var x = table.rows[row_counter].cells;
			if(parseInt(x[2].innerHTML)>parseInt(x[3].innerHTML)){
				x[4].innerHTML = "CU Buffs";
				document.getElementById("wins").innerHTML++;
				//Set winners column (buffs)
				//increment wins column, use ID

			}
			else {
				x[4].innerHTML = x[1].innerHTML;
				document.getElementById("losses").innerHTML++;
				//set winners column (opposing team)
				//increment losses column, use ID
			}
				...
				Span ids:
					...
					Calculated values:
					  ...
					  avg_rec_yards - the average number of receiving yards for the player's Buff career
						...
						NOTES:: players[3] is an array /ex) players[0] is John Doe's info,
						players[0].name is "John Doe" (accessing the name as an object)
						calculate the average ex) rushing yards/games played --> r_yards/g_played
	...
	function loadPlayersPage(){
		...
		var info="";
			for(var i=0; i<players.length; i++){
				...
				info = info+"<a class='dropdown-item' href='#' onclick='switchPlayers("+i+")'>"+player+"</a>"
				//console.log(player);
			}
